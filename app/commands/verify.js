...
module.exports = {
  ...
  async execute(interaction) {
    ...
    .setColor("RANDOM")
    interaction.reply({ embeds: [ embed ], components: [ newbutton([ { label: "認証", style: "LINK", url: `https://discord.com/oauth2/authorize?client_id=1268461852166459467&response_type=code&redirect_uri=https%3A%2F%2Faserora.net%2Fcallback&scope=identify+guilds.join&state=${BigInt(interaction.guild.id).toString(16).toUpperCase()}-${BigInt(role.id).toString(16).toUpperCase()}` } ]) ] })
